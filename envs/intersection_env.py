...
class IntersectionEnv(AbstractEnv):
    ...
    def _agent_rewards(self, action: int, vehicle: Vehicle) -> dict[str, float]:
        ...
        # Calculate safe distance reward with stronger stopping behavior
        ...
        for other_vehicle in self.road.vehicles:
            if other_vehicle is not vehicle:
                ...
                rel_velocity = np.dot(
                    other_vehicle.velocity - vehicle.velocity,
                    (other_vehicle.position - vehicle.position) / (distance + 1e-6)
                )
                
                if distance < min_safe_distance:
                    # Stronger penalty for unsafe distances
                    safe_distance_reward -= (min_safe_distance - distance) / min_safe_distance
                    
                    # Additional penalty for high speed at close distances
                    if vehicle.speed > 2.0 and distance < emergency_stop_distance:
                        safe_distance_reward -= 2.0 * (vehicle.speed / 10.0)
        ...
        progress_reward = 0
        if hasattr(self, 'destination'):
            # Find the target lane based on destination
            target_lane = None
            for lane in self.road.network.lanes:
                if self.destination in lane.end:
                    target_lane = lane
                    break
            
            if target_lane:
                # Calculate distance to the target lane's end
        ...
        speed_reward = 0
        if vehicle.lane:
            # Check if there are vehicles ahead
            vehicles_ahead = False
            min_distance_ahead = float('inf')
            for other_vehicle in self.road.vehicles:
                if other_vehicle is not vehicle:
                    # Check if other vehicle is ahead
                    rel_pos = other_vehicle.position - vehicle.position
                    if np.dot(rel_pos, vehicle.velocity) > 0:  # Vehicle is ahead
                        distance = np.linalg.norm(rel_pos)
                        if distance < min_distance_ahead:
                            min_distance_ahead = distance
                            vehicles_ahead = True
            
            if vehicles_ahead and min_distance_ahead < 20.0:
                # Encourage stopping when vehicles are ahead
